...
function displaySemesterSubjects() {
    ...
    semesterSubjects[currentSemester].forEach((subject, index) => {
        ...
        row.innerHTML = `
            <td class="border border-gray-300 dark:border-gray-600 p-2">${subject.name}</td>
            <td class="border border-gray-300 dark:border-gray-600 p-2">${subject.credit}</td>
            <td class="border border-gray-300 dark:border-gray-600 p-2">
                <input type="number" class="w-full grade-point bg-transparent" min="0" max="10" step="0.1" value="${semesters[currentSemester][index]?.gradePoint || ''}">
            </td>
            <td class="border border-gray-300 dark:border-gray-600 p-2 grade-letter"></td>
            <td class="border border-gray-300 dark:border-gray-600 p-2 score">0</td>
        `;
...
function calculateScore(e) {
    ...
    const credit = parseFloat(row.cells[1].textContent) || 0;
    const gradePoint = parseFloat(e.target.value) || 0;
...
function calculateCumulativeGpa() {
    ...
    const currentSemesterScore = parseFloat(document.getElementById('totalScore').textContent) || 0;
    const currentSemesterCredits = parseFloat(document.getElementById('totalCredits').textContent) || 0;
...
function saveSemesterData() {
    ...
    document.querySelectorAll('#gradeTable tr').forEach((row, index) => {
        semesterData.push({
            gradePoint: parseFloat(row.querySelector('.grade-point').value) || 0,
...
function showNotification(message) {
    ...
    notification.className = 'fixed top-4 right-4 bg-green-500 text-white px-4 py-2 rounded shadow-lg transition-opacity duration-300';
